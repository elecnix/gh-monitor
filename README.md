...
When GitHub rate-limits a PR watch's GraphQL query, the watch reads the PR over REST until GraphQL answers again ([#123](https://github.com/elecnix/gh-monitor/issues/123)). The REST and GraphQL budgets are separate, so REST usually has room left. The REST read covers state, merge, head commit, mergeability and check outcomes, so `merged`, `closed`, `new-commit`, `conflict`, `new-failing-checks` and `ci-all-green` still fire and `--until merged` still exits on the merge. It reads check suites as well as check runs, because a third-party suite that is queued or failed without any check runs is a pending or failing check, and the check-runs endpoint can't list it. Comments, review threads, reviews and annotations need GraphQL. They keep their last-known values while REST mode lasts, and the `degraded` notice that starts REST mode lists them in `degraded_surfaces`. That notice says "reading PR state over REST because GraphQL is exhausted until HH:MM" and carries the reset time in `degraded_reset_at`. Its `degraded_from` is the last successful GraphQL read. A second notice says when the watch reads over GraphQL again and closes the window with `degraded_from`/`degraded_to`, like the recovery notice above. The first GraphQL read after REST mode reports comments and threads still present, late. Changes that began and ended inside the window, and anything past the query's last 25 comments or threads, need a REST backfill. A merge or close read over REST ends the watch before GraphQL answers again, so no catch-up read runs. A caller that needs comments and reviews from that span reads them over REST starting at `degraded_from`. The daemon takes the budget from the `X-RateLimit-*` headers of its own API responses, because `GET /rate_limit` has reported a full GraphQL budget while real calls failed. It reads that endpoint only when no response in the current window gave a reading.
...
The `run-completed` event carries the run's `conclusion` (`success`, `failure`, `timed_out`, `cancelled`, `neutral`, `action_required`, `stale`, `skipped`) as structured JSON, plus `run_id`, the run URL, and the head commit. When the conclusion is a failure variant (`failure`, `timed_out`, `cancelled`, `action_required`), the event also carries a `detail` body holding the **last** 50 lines of the failed-job logs (`gh run view <run-id> --log-failed` puts the error at the end), so an agent can diagnose the failure without an extra turn. A longer log is truncated explicitly: the body's first line says how many earlier lines were dropped and names the exact command that fetches the whole failed-step log — `gh run view <run-id> --repo <owner>/<repo> --log-failed` — so nothing about the failure is out of reach. The same `--interval`, `--timeout`, `--once`, `--text`, and `-R` flags apply. `--run-id` is mutually exclusive with the PR selector and `--ref`/`--commit`/`--issue`.
...
**Health is loud, on purpose.** For a repository the broker is actually delivering events for (see [broker coverage](#broker-coverage-is-per-repository) — connection health alone is never enough), a quiet PR's idle-poll ceiling stretches from the default 300s up to `GH_MONITOR_BROKER_IDLE_CAP`; polling becomes a rare safety net because a real change now arrives as an immediate wake. The moment the connection drops, every subscriber gets a `degraded`-type notification and the ceiling reverts to the default within one poll cycle — normal interval polling, not silence. A subscriber that only ever reads "no event arrived" as "nothing changed" would be trading tonight's failure mode for a new transport instead of fixing it, so this transport is built to make that impossible: by default it always keeps polling underneath, just less often for the repositories the broker is demonstrably carrying. With `pollWhenBrokerHealthy: false` in preferences (see [Poll cadence](#poll-cadence)), that default becomes stricter still: covered repositories get no timer polling at all, and it returns the instant the transport degrades or their coverage lapses. Uncovered repositories keep the ordinary cadence under either setting.
