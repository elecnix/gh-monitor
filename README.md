...
When GitHub rate-limits a PR watch's GraphQL query, the watch reads the PR over REST until GraphQL answers again ([#123](https://github.com/elecnix/gh-monitor/issues/123)). The REST and GraphQL budgets are separate, so REST usually has room left. The REST read covers state, merge, head commit, mergeability and check outcomes, so `merged`, `closed`, `new-commit`, `conflict`, `new-failing-checks` and `ci-all-green` still fire and `--until merged` still exits on the merge. Comments, review threads, reviews and annotations need GraphQL. They keep their last-known values while REST mode lasts, and the `degraded` notice that starts REST mode lists them in `degraded_surfaces`. That notice says "reading PR state over REST because GraphQL is exhausted until HH:MM" and carries the reset time in `degraded_reset_at`. A second notice says so when the watch reads over GraphQL again. The daemon takes the budget from the `X-RateLimit-*` headers of its own API responses, because `GET /rate_limit` has reported a full GraphQL budget while real calls failed. It reads that endpoint only when no response in the current window gave a reading.
