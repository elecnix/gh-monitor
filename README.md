...
It works in both daemon modes ([#84](https://github.com/elecnix/gh-monitor/issues/84)): in polling mode an upgrade hands off seamlessly over the socket (below); in sub-daemon mode — where the children own their sockets and state — the launcher stops its children, starts the successor, and holds a grace window to confirm the successor stays up before exiting. A successor that fails to spawn or dies within that window is answered by relaunching the children on the old binary, so a sub-daemon fleet always converges to some running generation (at the cost of a brief gap where the polling-mode handoff has none).
