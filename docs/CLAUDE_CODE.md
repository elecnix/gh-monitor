...
| `await --until <event>`        | `monitor --until <kind,kind>` | Exits 0 the first time any listed event kind fires (triggering event reported first); exit 2 if the watch ends first.  |
