...
**GraphQL-budget aware:** the watcher reads the budget from the `X-RateLimit-*` headers of its own API responses (falling back to `rate_limit` only without a current reading) and stretches its cadence as the shared GraphQL budget runs low (below ~10% of the limit), slowing instead of contributing to wholesale exhaustion. Transitions into/out of the low state emit a `degraded` notification. When the budget is very low the snapshot query sheds low-priority surfaces in order — annotations, then reviews, then comments — keeping PR status and check outcomes alive in every tier; each shed is announced loudly, and shed surfaces keep their last-known values (a shed review never reads as dismissed). When GraphQL is rate limited, a PR watch reads state, merge, head commit, mergeability and check outcomes over REST, so `--until merged` still fires. A `degraded` notice announces the switch to REST and the switch back.
